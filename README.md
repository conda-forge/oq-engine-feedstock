About oq-engine-feedstock
=========================

Feedstock license: [BSD-3-Clause](https://github.com/conda-forge/oq-engine-feedstock/blob/main/LICENSE.txt)
...
Once the `conda-forge` channel has been enabled, `oq-engine` can be installed with `conda`:
...
```
mamba install oq-engine
```

It is possible to list all of the versions of `oq-engine` available on your platform with `conda`:
...
```
mamba search oq-engine --channel conda-forge
```
...
mamba repoquery search oq-engine --channel conda-forge

# List packages depending on `oq-engine`:
mamba repoquery whoneeds oq-engine --channel conda-forge

# List dependencies of `oq-engine`:
mamba repoquery depends oq-engine --channel conda-forge
```
